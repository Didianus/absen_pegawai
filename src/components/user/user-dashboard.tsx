...
import { Clock, LogIn, LogOut, CheckCircle, CalendarDays, TrendingUp, AlertTriangle } from 'lucide-react'
...
export function UserDashboard() {
  ...
  const handleCheckOut = async () => {
    ...
    setError(null)
    try {
      const res = await fetch('/api/attendance/check-out', { method: 'PUT' })
      const data = await res.json()
      if (res.ok) {
        setTodayRecord(data)
        await fetchStats()
