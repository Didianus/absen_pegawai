...
import { Plus, Search, ChevronLeft, ChevronRight, CalendarIcon, ArrowUpCircle, TrendingDown } from 'lucide-react'
...
interface BarangKeluarRecord {
  ...
  pengeluar: { id: string; name: string }
}
...
export function AdminBarangKeluar() {
  ...
  const [totalItemKeluar, setTotalItemKeluar] = useState(0)
  ...
  const fetchRecords = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/barang-keluar?${params.toString()}`)
      if (res.ok) {
        const data = await res.json()
        setRecords(data.records)
        setPagination(data.pagination)
        const allRecords = data.records as BarangKeluarRecord[]
        setTotalKeluar(allRecords.reduce((sum, r) => sum + r.jumlah, 0))
        setTotalItemKeluar(allRecords.length)
      }
    } catch {
      // silently fail
    } finally {
  ...
  const submitAdd = async (e: FormEvent) => {
    ...
    try {
      ...
      setAddOpen(false)
      await fetchRecords()
      toast({ title: 'Berhasil', description: 'Barang keluar berhasil dicatat' })
    } catch {
  ...
  const filtered = search
    ? records.filter((r) =>
        ...
        r.pengeluar.name.toLowerCase().includes(search.toLowerCase()) ||
        r.penerimaNama.toLowerCase().includes(search.toLowerCase())
      )
    : records
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <h1 className="text-xl font-semibold text-slate-900">Laporan Barang Keluar</h1>
        <Button onClick={handleAddOpen} className="shrink-0 bg-amber-600 hover:bg-amber-700">
        ...
        <Card className="border-amber-200">
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-slate-500">Total Barang Keluar</p>
                <p className="text-2xl font-bold text-slate-900">{totalKeluar}</p>
                <p className="text-xs text-slate-400">unit dari {totalItemKeluar} transaksi</p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-slate-400">catat barang keluar</p>
              </div>
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead className="hidden lg:table-cell">Dikeluarkan Oleh</TableHead>
      ...
      <Dialog open={addOpen} onOpenChange={setAddOpen}>
        <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>Tambahkan catatan barang yang keluar dari inventaris.</DialogDescription>
          </DialogHeader>
          <form onSubmit={submitAdd} className="grid gap-4 py-2">
            ...
            <div className="grid gap-2">
              <Label htmlFor="bk-barang">Pilih Barang</Label>
              <Select value={formBarangId} onValueChange={setFormBarangId}>
                <SelectTrigger id="bk-barang">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div className="grid gap-2">
                <Label htmlFor="bk-jumlah">Jumlah</Label>
                <Input id="bk-jumlah" type="number" min="1" max={selectedBarang?.stok || undefined} value={formJumlah} onChange={(e) => setFormJumlah(e.target.value)} placeholder="0" autoFocus required />
              </div>
              <div className="grid gap-2">
                <Label htmlFor="bk-tanggal">Tanggal</Label>
                <Input id="bk-tanggal" type="date" value={formTanggal} onChange={(e) => setFormTanggal(e.target.value)} required />
              </div>
            </div>
            <div className="grid gap-2">
              <Label htmlFor="bk-penerima">Nama Penerima</Label>
              <Input id="bk-penerima" value={formPenerimaNama} onChange={(e) => setFormPenerimaNama(e.target.value)} placeholder="Nama orang/divisi yang menerima barang" />
            </div>
            <div className="grid gap-2">
              <Label htmlFor="bk-keterangan">Keterangan</Label>
              <Input id="bk-keterangan" value={formKeterangan} onChange={(e) => setFormKeterangan(e.target.value)} placeholder="Alasan/keterangan barang keluar (opsional)" />
            </div>
            <DialogFooter>
